...
contract MoatPayment {
    ...
    modifier isValidator(string calldata moatName) {
        require(
            moats[moatName].validator == msg.sender,
            "MoatPayment: you are not the validator for this moat."
        );
        _;
    }
    ...
    function createMoat(string calldata moatName, address validator) public {
        require(
            moats[moatName].creator == address(0x0),
            "MoatPayment: moat already exists."
        );
     ...
     *  Adds fund to a moat.
     */
    function fundMoat(string calldata moatName, uint256 amt) public payable {
        Moat storage moat = moats[moatName];
        require(
            moat.validator != address(0x0),
            "MoatPayment: moat does not exist."
        );
     ...
     */
    function withdrawFromPool(string calldata moatName, uint256 amt)
        public
        isValidator(moatName)
    {
        Moat storage moat = moats[moatName];
        require(
            moat.pool >= amt,
            "MoatPayment: not enough value in moat's pool."
        );
     ...
     */
    function transferValidator(string calldata moatName, address newValidator)
        public
        isValidator(moatName)
    {
        require(
            newValidator != address(0x0),
            "MoatPayment: do not transfer validator to the zero address."
        );
